    ...
    getOperation()->walk([&](Operation *op) {
      llvm::TypeSwitch<Operation *>(op)
          ...
          .Case([&](gpu::ShuffleOp shuffle) {
            // Both a shuffle and a rotate require the first `width` lanes of
            // the subgroup to be active, which only holds when the whole
            // subgroup reaches the operation.
            checkExecution(shuffle, UniformityScope::Subgroup, "'gpu.shuffle'");
            checkSubgroupUniformOperand(shuffle, shuffle.getWidth(),
                                        "the width of 'gpu.shuffle'");
          })
          .Case([&](gpu::RotateOp rotate) {
            checkExecution(rotate, UniformityScope::Subgroup, "'gpu.rotate'");
          })
