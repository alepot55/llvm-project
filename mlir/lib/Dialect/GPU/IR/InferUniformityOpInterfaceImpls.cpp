...
/// differ between threads. A workgroup attribution is the same buffer for every
/// thread of the workgroup; a private attribution is a different buffer for
/// every thread.
...
/// received a value depends on the lane. In `idx` mode the second operand is
/// the lane every lane reads from, so a lane index that is the same across the
/// subgroup makes the result the same across the subgroup whatever the value
/// was; in the other modes it is a per-lane delta, each lane reads a different
/// lane and a uniform operand says nothing. The width must be the same on
/// every lane, but that is a precondition, not something a shuffle
/// establishes; the lanes at and above it, whose result the operation leaves
/// unspecified, are read here as not breaking uniformity.
///
/// Nothing is reported while an operand a rule reads is still uninitialized:
/// the join is a meet, so a scope taken from the value alone could not be
/// widened once the lane index settles.
void ShuffleOp::inferUniformity(ArrayRef<Uniformity> operandUniformity,
  ...
  if (value.isUninitialized())
    return;
  UniformityScope scope = value.getScope();
  if (getMode() == ShuffleMode::IDX) {
    Uniformity lane = operandUniformity[1];
    if (lane.isUninitialized())
      return;
    // A lane index that is the same across a wider group still only says that
    // the lanes of one subgroup all read the same lane of that subgroup.
    UniformityScope viaLane = meet(lane.getScope(), UniformityScope::Subgroup);
    if (scope < viaLane)
      scope = viaLane;
  }
  setUniformity(getShuffleResult(), scope);
}

/// A rotate moves every lane's value to another lane, and its offset and width
/// are attributes, so it has no operand that could make a divergent value
/// uniform the way a `gpu.shuffle idx` does.
